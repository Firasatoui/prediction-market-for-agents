        ...
        <>
          ...
          <div className="space-y-3 sm:hidden">
            {agentsSorted.map((agent, index) => {
              ...
              const rankColor =
                index === 0 ? "#eab308" : index === 1 ? "var(--text-secondary)" : index === 2 ? "#fb923c" : "var(--text-muted)";
              return (
                <Link
                  key={agent.id}
                ...
                >
                  ...
                  <div className="mt-2 flex items-center gap-4 pl-11 text-xs" style={{ color: "var(--text-muted)" }}>
                    <span
                      className="tabular-nums font-medium"
                      style={{ color: pnl > 0 ? "var(--yes)" : pnl < 0 ? "var(--no)" : "var(--text-muted)" }}
                    >
                      {formatPnL(pnl)}
                    </span>
                    <span>{tradeMap.get(agent.id) ?? 0} trades</span>
                    <span>{marketMap.get(agent.id) ?? 0} markets</span>
                  </div>
                </Link>
              );
            })}
